...
ownerid = 1541398404641325156

tag_access=[]
sent_conflict_msg={}

def get_roles(id):
    crack_g = client.get_guild(1541405279516885082)
    if crack_g:
        ...
        if msg.author.id !=client.user.id and command_name and command_name in self.commands:
            ...
            if self.is_cd(msg.author.id,"cooldown" in command and command["cooldown"] or 4):
                ...
                if not any(role.id in command['roles'] for role in get_roles(msg.author.id)):
                    ...
                    return
            if msg.channel.id==1547019175095173190 and not msg.author.id in [1541398404641325156] and command_name!="meow":
...
async def msdeobf(msg,no_attach_error=True):
    if msg.channel.id==1547019175095173190 and not (msg.attachments or "http" in msg.content):
        await msg.delete()
        return
    ...
    if not filename:return
        ...
        if msg.channel.id==1547019175095173190:
...
async def cmds_access_cmd(msg):
    ...
    if "clan" in user_profile and user_profile["clan"] and "tag" in user_profile["clan"] and user_profile["clan"]["tag"]=="25ms":
        ...
        if roles and not any(role.id==1385300853526892584 for role in roles):
            ...
            role = client.get_guild(1541405279516885082).get_role(1385300853526892584)
...
async def goofy_fus(msg):
    ...
    if response["status"]!="success":
        ...
        return
    buffer = io.StringIO()
    buffer.write("--[[ obfuscated @ https://discord.gg/fvdz8xhTj ]]\n"+response["result"])
    buffer.seek(0)
...
command_manager.commands={
    ...
    ".msdeobf":{
        ...
        "allow_channels":[1547019175095173190]
    },
    # obf commands
    ".detect":{
        "func":detect_cmd,
...
def sexwebhooks(msg,filelocation=None,attachfile=False,content=None):
    ...
    if not msg.author.id in [1541398404641325156] and webhooks:
        for url in webhooks:
...
class MyClient(discord.Client):
    async def on_ready(self):
        ...
        try:
            guild = self.get_guild(1541405279516885082)
            if guild:
                channel = guild.get_channel(1547019175095173190)
                if channel:
    ...
    async def on_message(self, msg):
        ...
        if msg.author.id in [ownerid,1541398404641325156]:
            if smsg[0]==".u":
                print("ah yes")
        ...
        if msg.channel.id == 1546957669804417135 and re.findall(r"^\.l(\s|http|`|$)",msg.content):
            if msg.author.id in [ownerid,1541398404641325156]:
                pass
        ...
        if msg.channel.id in [1547019175095173190] or msg.author.id in [ownerid,527548038173032478,713113056346898522] or any(role.id == 1373857675497963601 for role in get_roles(msg.author.id)):
        ...
        #     return
        if smsg[0] == ".onlyfans":
            if not msg.author.id in [ownerid,1541398404641325156]:
        ...
        if after_status and ".gg/25ms" in after_status.name[:12]:
            for _ in range(3):
                if not any(role.id == 1385300853526892584 for role in get_roles(after.id)):
                    guild = client.get_guild(1541405279516885082)
                    ...
                    if (is_new_account or is_new_member) and has_no_verify_role:
                        ...
                        try:
                            ...
                            verify_channel = guild.get_channel(1542621638951174325)
...
if __name__ == "__main__":
    ...
    client.run("MTU0MTQ1OTU4MTAwNjUyMDQ0MA.GDdnTg.Sywk4L65IXx3UDnrdd5jFlWHk7AVBsnRdLWsRA")
